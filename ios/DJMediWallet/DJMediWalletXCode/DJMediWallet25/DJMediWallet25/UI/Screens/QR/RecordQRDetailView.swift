...
struct RecordQRDetailView: View {
    let record: RecordItem
    let navigationTitle: String
    let missingResourceMessage: String
    let bundleBuilder: (RecordItem) throws -> FHIRBundle
    ...
    private func handleError(_ error: Error) {
        if let decodingError = error as? FHIRResourceDecodingError {
            switch decodingError {
            case .missingData:
                errorMessage = missingResourceMessage
            default:
                errorMessage = decodingError.errorDescription ?? decodingError.localizedDescription
            }
        } else if let localError = error as? LocalizedError, let description = localError.errorDescription {
    ...
    private enum ViewError: LocalizedError {
        ...
        var errorDescription: String? {
            "Unable to render the QR code for this record."
        }
    }
}
